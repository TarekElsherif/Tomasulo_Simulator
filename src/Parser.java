...
public class Parser {
	...
	public Parser(String i, String d) {
		...
		try {
			...
			insAddress = Integer.parseInt(fline[1]);
			String[] fline2 = data[0].split(" ");
			dataAddress = Integer.parseInt(fline2[1]);
		} catch(Exception e) {
			System.out.println("Input Address is invalid - Parser Class");
		}
		instructions = bufferToInstructions(Arrays.copyOfRange(ins, 1, ins.length));
		bytes = bufferToByte(Arrays.copyOfRange(ins, 1, ins.length));
	}
	...
	public ArrayList<Byte> bufferToByte(String[] s) {
		...
		for (int i = 0; i < s.length; i++) {
			Byte b = new Byte(Integer.parseInt(s[i]));
		}
		return result ;
	}
	...
	public static void main(String [] args) {
		String i = ".ORG 300\nLW R1, 24(R2)\nADD R3, R4, R5";
		String d = ".ORG 200\n24\3\16";
		Parser p = new Parser(i, d);
		for (int j = 0; j < p.instructions.size(); j++) {
			Instruction x = p.instructions.get(j);
			System.out.println(x.getOp() + " " + x.getDestReg() + " " + x.getSrcReg() + " " + x.getSrcReg2() + " " + x.getImmediate());
		}
	}
	
	public ArrayList<String> splitToStrings(String[] array) {
		return null;
